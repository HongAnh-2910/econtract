...
@section('content')
    <style>
        .box__select--application--edit {
            border: 1px solid #dce7f1;
            border-radius: 5px;
        }

        .box__select--application--edit .select2-selection--single {
            height: 36px;
            border: none;
        }

        .box__select--application--edit .select2-selection__rendered {
            height: 100%;
            display: flex !important;
            align-items: center;
        }

        .box__select--application--edit .select2-selection__arrow {
            height: 100% !important;
            display: flex !important;
            align-items: center;
        }

        .box__select--application--edit .select2-container {
            min-width: 100%;
    ...
    <form method="POST" action="{{ route('web.applications.update', $application->id) }}" class="needs-validation" novalidate>
        @csrf
        <div class="container-fluid">
            <div class="row px-3">
                <div class="col-md-8 col-12 mt-5 px-0">
                    <div class="row">
                        <div class="col-10 col-md-5">
                            <label class="mr-5 mb-2">Lý do <span class="text-danger">*</span></label>
                            <div class="box__select--application--edit">
                                <select name="reason" class="form-control select2 w-100" style="width: 100% !important;">
                                    <option @if ($application->reason == 'Nghỉ ốm') selected @endif data-foo="Tối đa: 30 Ngày/ Năm"
                                        value="Nghỉ ốm">Nghỉ ốm</option>
                                    <option @if ($application->reason == 'Nghỉ thai sản') selected @endif data-foo="Tối đa: 180 Ngày/ Năm"
                                        value="Nghỉ thai sản">Nghỉ thai sản</option>
                                    <option @if ($application->reason == 'Nghỉ không lương') selected @endif data-foo="Tối đa: 20 Ngày/ Năm"
                                        value="Nghỉ không lương">Nghỉ không lương</option>
                                    <option @if ($application->reason == 'Nghỉ phép năm') selected @endif data-foo="  " value="Nghỉ phép năm">Nghỉ phép
                                        năm
                                    </option>
                                    <option @if ($application->reason == 'Nghỉ khác') selected @endif data-foo="Tối đa: 3 Ngày/ Năm"
                                        value="Nghỉ khác">
                                        Nghỉ khác</option>
                                    <option @if ($application->reason == 'Nghỉ con ốm') selected @endif data-foo="Tối đa: 20 Ngày/ Năm"
                                        value="Nghỉ con ốm">Nghỉ con ốm</option>
                                    <option @if ($application->reason == 'Nghỉ dưỡng sức sau ốm đau') selected @endif data-foo="Tối đa: 10 Ngày/ Năm"
                                        value="Nghỉ dưỡng sức sau ốm đau">Nghỉ dưỡng sức sau ốm đau</option>
                                    <option @if ($application->reason == 'Nghỉ hội nghị, học tập') selected @endif data-foo="Tối đa: 7 Ngày/ Năm"
                                        value="Nghỉ hội nghị, học tập">Nghỉ hội nghị, học tập</option>
                                    <option @if ($application->reason == 'Nghỉ dưỡng sức sau thai sản') selected @endif data-foo="Tối đa: 30 Ngày/ Năm"
                                        value="Nghỉ dưỡng sức sau thai sản">Nghỉ dưỡng sức sau thai sản</option>
                                    <option @if ($application->reason == 'Nghỉ dưỡng sau điều trị thương tật, tai nạn') selected @endif data-foo="Tối đa: 7 Ngày/ Năm"
                                        value="Nghỉ dưỡng sau điều trị thương tật, tai nạn">Nghỉ dưỡng sau điều trị thương
                                        tật,
                                        tai nạn</option>
                                    <option @if ($application->reason == 'Nghỉ bù') selected @endif data-foo="Đơn vị tính theo giờ"
                                        value="Nghỉ bù">Nghỉ bù</option>
                                    <option @if ($application->reason == 'Nghỉ tai nạn') selected @endif data-foo="Tối đa: 30 Ngày/ Năm"
                                        value="Nghỉ tai nạn">Nghỉ tai nạn</option>
                                    <option @if ($application->reason == 'Nghỉ hiếu hỷ') selected @endif data-foo="Tối đa: 3 Ngày/ Năm"
                                        value="Nghỉ hiếu hỷ">Nghỉ hiếu hỷ</option>
                                    <option @if ($application->reason == 'Gặp khách hàng') selected @endif data-foo="Tối đa: 3 Ngày/ Năm"
                                        value="Gặp khách hàng">Gặp khách hàng</option>
                                </select>
                            </div>
                        </div>
                        <script>
                        ...
                        <input name="department_id" value="Phòng kĩ thuật" hidden>
                                ...
                                <div class="col-5 col-xl-2">
                                    ...
                                    <select class="form-select" name="information_day_1[]" required>
                                        <option class="application__selectOption--disabled" disabled="disabled"
                                            selected="selected">Chọn ca</option>
                                        <option @if ($informationDay->information_day_1 == 'Ca sáng') selected @endif value="Ca sáng">Ca sáng</option>
                                        <option @if ($informationDay->information_day_1 == 'Ca chiều') selected @endif value="Ca chiều">Ca chiều</option>
                                    </select>
                                </div>
                                <div class="col-5 col-xl-3 px-0">
                                    <label class="mb-2">Từ ngày<span class="text-danger">*</span></label>
                                    ...
                                    ?>
                                    <input class="form-control" type="date" name="information_day_2[]"
                                        value="{{ date('Y-m-d', strtotime($informationDay->information_day_2)) }}">
                                </div>
                                <div class="col-5 col-xl-2 mt-3 mt-xl-0">
                                    <label class="mb-2">Ca<span class="text-danger">*</span></label>
                                    <select class="form-select" name="information_day_3[]" required>
                                        <option class="application__selectOption--disabled" value="" disabled="disabled"
                                            selected="selected">Chọn ca</option>
                                        <option @if ($informationDay->information_day_3 == 'Ca sáng') selected @endif value="Ca sáng">Ca sáng</option>
                                        <option @if ($informationDay->information_day_3 == 'Ca chiều') selected @endif value="Ca chiều">Ca chiều</option>
                                    </select>
                                </div>
                                <div class="col-5 col-xl-3 mt-3 mt-xl-0 px-0">
                                    <label class="mb-2">Đến ngày<span class="text-danger">*</span></label>
                                    <input class="form-control" type="date" name="information_day_4[]"
                                        value="{{ date('Y-m-d', strtotime($informationDay->information_day_4)) }}">
                                </div>
                                <i class="fas fa-times delete__data--button mt-0 mt-xl-4"
                                    style="width:35px; height:40px;"></i>
                            </div>
                        @endforeach
                    @endif
                    ...
                    <div class="col-12 col-md-10 mt-4 px-0">
                        ...
                        <textarea class="ckeditor" required name="description">{{ $application->description }}</textarea>
                    ...
                    <div class="col-12 col-md-10 mt-4 px-0 application__file--customize">
                        <div class="application__file--inputFile">
                            <label for="select_file" class="mb-3">
                                <div class="btn rounded ml-2 px-2 mb-md-0" data-bs-target="#uploadFileModal"
                            ...
                            <div class="col-12 col-md-6 mt-3 mt-md-0">
                                <select class="form-control onChangeUser" name="user_id">
                                    @foreach ($users as $user)
                                        <option @if ($application->user_id == $user->id) selected @endif value="{{ $user->id }}">
                                            {{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
        ...
        <div class="footer__application--style w-100 position-fixed"
            ...
            <div class="col-12 mt-2 mx-0 form-group application__for--thought">
                ...
                <button onclick="window.location.href='{{ url()->previous() }}'" type="button"
                    class="btn btn-outline-primary">Thoát</button>
            </div>
        </div>
    </form>
